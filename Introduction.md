OpenCV（开源机器视觉库：[http://opencv.org](http://opencv.org ""))是一个基于BSD协议的开源库，它包含了上百种机器视觉算法。本篇文档介绍了OpenCV 2.x的API使用，提供了C++风格的API，而不是OpenCV 1.x的C语言风格（从2.4版本开始，C语言版本的API已经被弃用。并且这些代码没有用C编译器进行测试）。

OpenCV提供了模块化的结构，也就说，OpenCV是由许多动态库或静态库组成的。OpenCV包含了以下子库：

* 核心库（core） ：压缩的模块，定义了基本的数据结构，其中包括高纬度的数组Mat和提供给其他库使用的基本函数。

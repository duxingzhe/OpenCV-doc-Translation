...
我们能从现实世界的很多地方获得图片：数码相机、扫描仪、机器体层扫描仪和核磁共振仪等。在上述情形中，我们（人类）看到的是图片。但对于，我们的机器而言，我们记录的只不过是图像的各种各样像素点的数值而已。

![](https://docs.opencv.org/4.1.0/MatBasicImageForComputer.jpg)

就像上面的汽车图片，对于计算机而言，他就是一个矩阵，矩阵的元素代表的是密密麻麻像素点的数值。我们根据自身的需求去存储并使用其中的值。但是最后，在计算机领域中，每一张图像都要最终转化为数值矩阵，还有其他能够描述矩阵的信息。OpenCV是开源的机器视觉处理库，它主要功能便是用来生成图像数字矩阵和其他描述矩阵的内容，并对这些数据进行处理。由此，第一步，你应该对OpenCV如何存储和处理这些图像有非常熟悉的了解。

Mat

自2001年起，OpenCV开始进行持续开发。在当时，OpenCV是用C实现的，提供C语言接口，是通过IplIMage的C语言结构体来存储在内存里。因此，一些比较老的教程或者教科书里用的是OpenCV的C语言使用方式。然而，C语言带来一个问题，那就是C语言本身做的事情非常有限。最大的问题是我们需要对内存进行手动管理。C语言假定程序员应该对内存分配和回收负责。当然，这对于小程序而言没有多大问题。但是，一旦项目变得庞大起来，程序开发便会从目标功能实现被迫转向异常繁琐的内存管理上去。

庆幸的是，后来开发了C++，并引入类的概念，这让用户可以自动化处理内存占用，从而简化内存管理。而且有一个好消息，C++对C语言是完全兼容的，所以将OpenCV改成C++并不会引发任何问题。因此，从OpenCV 2.0开始，我们便引入了新的C++接口，这样我们为用户提供了新的编程方法，从而让用户不必受困于内存管理的麻烦当中，让用户的代码更加简明扼要（写的更少，做的更多）。C++接口的一个弊端是许多嵌入式开发系统目前只支持C。因此，如果你在使用嵌入式系统，请使用C语言开发。如果你并不是使用嵌入式设备，则不要使用旧方法（除非你是一位喜欢给自己挖坑的程序员，你这样做就是在自找麻烦）。
